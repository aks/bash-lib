...
void parse_init(int argc, char** argv, char* buffer, int buf_size) {
  int i;
  buffer[0] = 0;
  for (i=0; i<argc; i++) {
    if (i == 0 )
...
void parse_dates(const char *inputstr);

int valid_number (const char *numstr, int min, int max, char *errmsg) {
  int num = atoi(numstr);
  if (num < min || num > max)
    errorf(errmsg, numstr);
  return num;
}

int valid_year (const char *ystr) {
  return valid_number(ystr, -4712, 3267, "Bad year value: '%s'\n");
}

int valid_month (const char *mstr) {
  return valid_number(mstr, 1, 12, "Bad month value: '%s'\n");
}

int valid_month_name (const char *mstr) {
  static char *month_names[] = {
    "January", "February", "March", "April", "May", "June", "July",
    "August", "September", "October", "November", "December" };
  int i;
  for (i=0; i<=11; i++) {
    if (strncasecmp(month_names[i],mstr,strlen(mstr)) == 0) return i+1;
  }
  errorf("Bad month name: '%s'\n", mstr);
  return 0;
}

int valid_weekday_name(const char *dstr) {
  static char *day_names[] = { 
    "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"
  };
  int i;
  for (i=0;i<7;i++) {
    if (strncasecmp(day_names[i],dstr,strlen(dstr)) == 0) return i;
  }
  return -1;
}

int valid_day (const char *dstr) {
  return valid_number(dstr, 1, 31, "Bad day value: '%s'\n");
}

char *valid_time (const char *timestr) {
  return (char *)timestr;
}
char *valid_timezone (const char *tzonestr) {
  return (char *)tzonestr;
}
...
int parse_date_yyyy_mm_dd(int argc, const char *args[]) {
  year  = valid_year(args[1]);
  month = valid_month(args[3]);
  day   = valid_day(args[4]);
  return 1;
}

int parse_date_mm_dd_yyyy(int argc, const char *args[]) {
  month = valid_month(args[1]);
  day   = valid_day(args[3]);
  year  = valid_year(args[4]);
  return 1;
}

int parse_date_dd_mm_yyyy(int argc, const char *args[]) {
  day   = valid_day(args[1]);
  month = valid_month(args[3]);
  year  = valid_year(args[4]);
  return 1;
}
...
int parse_date_dd_mmm_yyyy(int argc, const char *args[]) {
  day   = valid_day(args[1]);
  month = valid_month_name(args[3]);
  year  = valid_year(args[4]);
  return 1;
}
int parse_date_mmmm_dd_yyyy(int argc, const char *args[]) {
  month = valid_month_name(args[1]);
  day   = valid_day(args[3]);
  year  = valid_year(args[4]);
  return 1;
}
...
int parse_date_unix(int argc, const char *args[]) {
  weekday   = valid_weekday_name(args[1]);
  month     = valid_month_name(args[2]);
  day       = valid_day(args[3]);
  timestr = (char *)valid_time(args[4]);             // we don't validate time here
  year      = valid_year(args[5]);
  tzone   = (char *)valid_timezone(args[6]);         // not much real validation
  return 1;
}
...
/* these functions take a string to parse for futher args */

int parse_julian_date_arg(int argc, const char *argstr) {
  parse_dates(argstr);
  jdn = date_to_jdn(GREGORIAN,year, month, day);
  printf("%d\n", jdn);
  return 1;
}

int parse_date_jdn_arg(int argc, const char *argstr) {
  parse_dates(argstr);
  // TODO: compute date from jdn
  return 1;
}

int parse_weeknum_arg(int argc, const char *argstr) {
  parse_dates(argstr);
  // TODO: compute the weeknum from the date
  return 1;
}
int parse_weekday_arg(int argc, const char *argstr) {
  parse_dates(argstr);
  // TODO: compute the weekday from the parse date
  return 1;
}

int parse_jdn(int argc, const char *argstr) {
  jdn = atoi(argstr);
  // TODO: compute DATE from JDN
  return 1;
}
int parse_leapyear_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: calculate leapyear
  return 1;
}
int parse_solarnum_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: calculate solarnum
  return 1;
}
int parse_epact_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: calculate epact
  return 1;
}
int parse_paschal_moon_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: compute pascal_moon
  return 1;
}
int parse_easter_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: compute easter for given year
  return 1;
}
int parse_easter_sunday_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: compute easter sunday (gregorian) for given year
  return 1;
}
int parse_julian_period_arg(int argc, const char *argstr) {
  year = atoi(argstr);
  // TODO: compute easter sunday (gregorian) for given year
  return 1;
}

int show_command_help(int argc, const char *argstr) {
  talk(help_text);
  return 1;
}
...
void parse_dates(const char *inputstr) {
  ...
  if (rc == 0 || func == NULL) {
    errorf("Don't understand this: '%s'\n", inputstr);
  }
  (*func)(matches, date_parts_list);        // call the function with parsed list of strings
}

/* keywords */

typedef enum { YEAR_ARG, DATE_ARG, JDN_ARG, NO_ARG } ArgType;
...
KW_item parse_keywords[] =  {
  { "julian",       "j",  &parse_julian_date_arg,   DATE_ARG },
  { "date",         "d",  &parse_date_jdn_arg,      JDN_ARG  },
  { "weekday",      "wd", &parse_weekday_arg,       DATE_ARG },
  { "weeknum",      "wn", &parse_weeknum_arg,       DATE_ARG },
  { "leapyear",     "ly", &parse_leapyear_arg,      YEAR_ARG },
  { "solarnum",     "sn", &parse_solarnum_arg,      YEAR_ARG },
  { "epact",        "ep", &parse_epact_arg,         YEAR_ARG },
  { "paschalmoon",  "pm", &parse_paschal_moon_arg,  YEAR_ARG },
  { "easter",       "e",  &parse_easter_arg,        YEAR_ARG },
  { "eastersunday", "es", &parse_easter_sunday_arg, YEAR_ARG },
  { "julianperiod", "jp", &parse_julian_period_arg, YEAR_ARG },
  { "help",         "h",  &show_command_help,       NO_ARG   },
...
int parse_keyword(const char *input) {
  ...
  if (type != NO_ARG) {
    ...
    if (rc <= 0) errorf("Keyword '%s' ('%s') needs an argument.\n", keyword, matched_keyword);
...
void parse_action(char * input_buffer) {
  ...
  if (0 == rc) {                          // if no keyword, then
    parse_dates(input_buffer);            // try to match the input buffer for dates
  }
}
...
void show_date(int year, int month, int day) {
  ...
  if (chars > 0) {
    ...
    errorf("Error in strftime: datestr = '%s'\n", datestr);
  }
}
...
int main (int argc, char** argv) {
  ...
  parse_init(argc, argv, parse_buffer, BUF_SIZE);
